...
const FPrimaryAssetType UNiumaItemDefinition::AssetType(TEXT("NiumaItemDefinition"));

FPrimaryAssetId UNiumaItemDefinition::GetPrimaryAssetId() const
{
    if (ItemId.IsNone())
    {
        // 没有稳定 ItemId 的资产不能注册为有效主资产。
        return FPrimaryAssetId();
    }

    return FPrimaryAssetId(AssetType,ItemId);
}
